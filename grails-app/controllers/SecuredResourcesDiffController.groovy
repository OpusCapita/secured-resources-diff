    ...
    def diff(SecuredResourcesDiffCommand command) {
        ...
        if (command.validate()) {
            ...
            if (oldResources == null) {
                command.errors.rejectValue('versionFrom', 'versionFrom.invalid', [command.versionFrom, app[1]] as Object[], 'Application "{1}" with version "{0}" doesn\'\'t exists')
            } else {
                List newResources = securedResourcesDiffService.loadSecuredResources(app[0], app[1], command.versionTo)
                if (newResources == null) {
                    command.errors.rejectValue('versionTo', 'versionTo.invalid', [command.versionTo, app[1]] as Object[], 'Application "{1}" with version "{0}" doesn\'\'t exists')
                } else {
