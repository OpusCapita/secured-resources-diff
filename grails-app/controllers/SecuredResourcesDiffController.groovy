...
class SecuredResourcesDiffController {
    ...
    def index() {
        ...
        if (!command.application) {
            ...
            command.application = "${application.artefactId}"
        }
        command.versionFrom = ""
        command.versionTo = ""

        String[] app = command.application.split(':')
        List versions = securedResourcesDiffService.getListOfVersions(app[0], command.showOnlyReleases)
    ...
    def diff(SecuredResourcesDiffCommand command) {
        ...
        if (command.validate()) {
            ...
            List oldResources = securedResourcesDiffService.loadSecuredResources(app[0], command.versionFrom)

            if (oldResources == null) {
                command.errors.rejectValue('versionFrom', 'versionFrom.invalid', [command.versionFrom, app[0]] as Object[], 'Application version "{0}" doesn\'\'t exist')
            } else {
                List newResources = securedResourcesDiffService.loadSecuredResources(app[0], command.versionTo)
                if (newResources == null) {
                    command.errors.rejectValue('versionTo', 'versionTo.invalid', [command.versionTo, app[0]] as Object[], 'Application  version "{0}" doesn\'\'t exist')
                } else {
        ...
        String[] app = command.application.split(':')
        model.versions = securedResourcesDiffService.getListOfVersions(app[0], command.showOnlyReleases)
